...
//
// role 매트릭스:
//   - admin     : 전체 HITLQueue pending+in_review 카운트 (institution 무관) + missionPendingToday=0
//   - principal : 본인 institutionId 의 HITLQueue (HITLQueue.user.institutionId == 본인) + missionPendingToday=0
//   - teacher   : 동일 (institution scope) — Teacher portal 노출용 + missionPendingToday=0
//   - expert    : assignedExpertId == userId 인 HITLQueue 만 (본인 담당 큐) + missionPendingToday=0
//   - parent    : HITL 카운트 0 + missionPendingToday = 본인 (R4) 오늘 미완료 SessionLog 카운트
//   - anonymous : 0 (호출 가드)
//
...
export const getNavBadgeCounts = cache(
  async (input: NavBadgeCountsInput): Promise<NavBadgeCounts> => {
    ...
    // parent: "오늘 미완료 미션" 카운트만 산출 (HITL 메뉴 미노출).
    if (role === "parent") {
      if (!userId) return empty;
      try {
        const missionPendingToday = await countMissionPendingToday(userId);
        return { hitlPending: 0, missionPendingToday };
      } catch (err) {
        // graceful: 본인 미션 count 실패 → 0 (nav 차단 금지).
        console.error(
