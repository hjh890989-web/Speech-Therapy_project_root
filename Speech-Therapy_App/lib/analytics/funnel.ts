...
// Performance 3차 (FR-PERF-3-USE-SERVER-REFACTOR 후속) — 순수 shape (타입 / 상수 /
// 순수 helper) 는 `./funnel-shape` 으로 분리되어 Client Component 가 안전하게 import.
// 본 파일은 prisma 의존 server-only helper. 기존 import 경로 호환을 위해 shape 의
// 모든 public symbol 을 re-export.
import {
  FUNNEL_STEP_ORDER,
  FUNNEL_STEP_LABEL,
  FUNNEL_CACHE_TAG,
...
// 기존 import path 호환 — server-only callers (RSC pages, cron, tests) 가 변경 없이
// 본 파일에서 types/helpers 를 계속 import 할 수 있도록 re-export.
export {
  FUNNEL_STEP_ORDER,
  FUNNEL_STEP_LABEL,
  FUNNEL_CACHE_TAG,
...
// (`safeRatio` / `buildSteps` 는 ./funnel-shape 으로 이동 + 본 파일에서 re-export.)
...
// `FUNNEL_CACHE_TAG` / `FUNNEL_CACHE_TTL_SECONDS` 는 ./funnel-shape 으로 이동 + 위 re-export.
...
// `StepDelta` / `compareConversions` / `FUNNEL_ALERT_THRESHOLD_*` / `FunnelAlertItem`
// / `pickAlertSteps` / `FUNNEL_STEP_LABEL` 모두 ./funnel-shape 으로 이동 + 위 re-export.
