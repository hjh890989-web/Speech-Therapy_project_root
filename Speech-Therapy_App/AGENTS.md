...
> 상위 참조(2트랙 정본): `docs/PRD_V11_TwoTrack.md`, `docs/SRS_V08_TwoTrack.md`, `docs/VPS_V10_TwoTrack_Realign.md`, `docs/realignment/00_2track_realignment_blueprint.md`. 이력(불변): 54_PRD_V10_Final / 65_SRS_V07.

---

## 1. 프로젝트 정체성

- **제품명**: Speech-Therapy — 부모용 **2트랙 비대칭** 아동 발달 가이드 서비스 (DTx 아님, 의료 보조 도구).
  - **트랙A — 발음·발화 발달 "확인"** (만 2~7세): 표준화 규준(U-TAP·SELSI) 기반 또래 비교 probe·점수·추이.
  - **트랙B — 읽기·말 "놀이·연습"** (만 2~12세): 점수·밴드·또래백분위·판정 **없음**(연습-only, `lib/literacy/stages.ts` 전 단계 `bandShippable=false`).
  - ⚠️ **불변: "확인(probe·또래비교)"은 발음(트랙A) 전용** — 문해(트랙B)는 "측정/평가/확인"이 아니라 "놀이·연습"으로만 표현. **"측정 vs 측정"이 아니다.**
  - 문해력(CR-2026-009, 5단계 사다리, 14게임): 임상·법률 게이트 통과 + **2026-06-22 공개 런치 완료**(`LITERACY_*_ENABLED` on, 만 2~12세 라이브).
- **단계**: MVP Phase 0 (스크리닝 + 가이드) → Phase 1 (리텐션, 보상) → Phase 2 (B2B 도입). 문해력 확장 = CR-2026-009.
- **북극성 KPI (2트랙, 2026-06-22 재정렬 결정 = C 지향·A 1차)**:
  - 트랙A **W-AUR (주간 발음 미션 완수율) ≥ 60%** — 발음 정본 지표(불변).
  - 트랙B **W-LER (주간 문해 활동률, engagement)** — 보조지표로 신설(완수율 아님) → baseline 축적 후 이원 북극성 동격 승격. ⚠️ 문해에 "완수율/미션" 프레임 금지.
- **타깃 페르소나**: 발음 발달이 걱정인 자녀(만 2~7세, 트랙A) + 읽기·말 발달을 집에서 함께하고픈 자녀(만 2~12세, 트랙B, 학령기 포함)의 부모 (Seg A~D + 트랙B 신규 페르소나, PRD §3)
...
- 모든 UI / 카피 / API 응답에서 다음 금칙어 사용 금지: **"치료"**, **"진단"**, **"장애"**
  - **문해(트랙B) 추가 금칙어**: **"학습장애"·"난독"·"읽기장애"·"지연"·"지체"** — 대안: "읽기·말 놀이/연습", "발달 단계에 맞춘 연습", "읽기·말이 아직 익숙해요".
- 대안 표현: "발음 발달 확인"(트랙A), "발음 가이드", "발음 어려움" / "읽기·말 놀이"(트랙B)
- **트랙B 연습-only 불변규칙**: 문해는 점수·밴드·또래백분위·정상/위험 판정·심각도·"측정/평가/확인(probe)" 카피 **금지**. 또래 비교 "확인"은 발음(트랙A)만 허용. 문해 산출은 engagement(활동 횟수·활동일)만, `referenceBand=null`. 학년/단계(stage) 라벨 대신 놀이명만 노출.
