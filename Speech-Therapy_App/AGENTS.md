...
- **제품명**: Speech-Therapy — 부모용 **발음/발화(만 2~7세) + 문해력(읽기·말, 만 2~12세)** 발달 확인 + 가이드 서비스 (DTx 아님, 의료 보조 도구)
  - 문해력(CR-2026-009, 5단계 사다리 `lib/literacy/stages.ts`)은 **게임별 플래그(LITERACY_*_ENABLED) 게이팅** — 임상·법률 게이트 통과(2026-06-22) 후 공개 런치 시 활성. 플래그 off 동안 제품 노출은 발음(만 2~7세)만.
...
- **타깃 페르소나**: 발음 발달이 걱정인 자녀(만 2~7세) + 읽기·문해 발달이 걱정인 자녀(만 2~12세)의 부모 (Seg A~D, PRD §3)
