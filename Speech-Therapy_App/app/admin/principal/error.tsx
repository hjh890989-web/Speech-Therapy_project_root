...
// FR-Q-009 — /admin/principal error boundary (Next.js 16 App Router error.tsx).
...
export default function PrincipalDashboardError({
  ...
  useEffect(() => {
    ...
    console.error(
      JSON.stringify({
        level: "error",
        event: "admin_principal_render_error",
  ...
  return (
    <main
      data-testid="admin-principal-error"
    ...
    >
      <div className="rounded-lg border border-rose-300 bg-rose-50 p-6 text-rose-900">
        ...
        <p className="mt-2 text-sm">
          데이터베이스 연결 또는 일시적인 오류로 기관 요약을 표시하지 못했어요.
          잠시 후 다시 시도해 주세요.
        </p>
        {error.digest ? (
          <p
