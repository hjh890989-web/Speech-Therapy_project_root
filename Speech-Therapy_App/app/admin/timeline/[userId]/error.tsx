...
// FR-Q-013 (#54) — /admin/timeline/[userId] error boundary.
// Server Component fetch (Prisma / Supabase) 실패 시 본 boundary 가 catch. reset() 으로 단순 재시도.
...
export default function TimelineError({
  ...
  useEffect(() => {
    // Vercel Logs / Drains 가 수집 — R4: 본문엔 자녀 식별자 미포함.
    console.error(
      JSON.stringify({
        level: "error",
        event: "admin_timeline_render_error",
  ...
  return (
    <main
      data-testid="admin-timeline-error"
    ...
    >
      <div className="rounded-lg border border-rose-300 bg-rose-50 p-6 text-rose-900">
        <h1 className="text-xl font-semibold">타임라인을 불러오지 못했어요</h1>
        <p className="mt-2 text-sm">
          데이터베이스 연결 또는 일시적인 오류로 활동 기록을 표시하지 못했어요. 잠시 후 다시
          시도해 주세요.
        </p>
        {error.digest ? (
          <p
