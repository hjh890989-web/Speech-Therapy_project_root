...
const SUPPORTED_LEVELS = [2, 3] as const;
...
describe("mission-content: 데이터 무결성", () => {
  it("5 자모 × 2 난이도 = 10 sets 존재", () => {
    expect(missionContentByPhonemeLevel.size).toBe(10);
  ...
  it("getMissionContent: 미지원 조합은 undefined", () => {
    ...
    expect(getMissionContent("ㅅ", 1)).toBeUndefined();
