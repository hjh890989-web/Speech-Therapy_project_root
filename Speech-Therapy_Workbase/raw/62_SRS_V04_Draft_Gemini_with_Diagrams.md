...
*   **D3. 재활사 풀(Pool)**: 48시간 내 피드백 SLA를 준수하기 위해 적정 수의 파트타임 언어재활사 풀 확보 및 배정 시스템이 상시 가동되어야 가동되어야 한다.
...
### 3.3 Core Architecture Diagrams

#### 3.3.1 UseCase Diagram
각 페르소나(Actor)가 시스템과 상호작용하는 핵심 유스케이스입니다.

```mermaid
flowchart LR
    A(불안형 부모) --> UC1([5분 스크리닝 진단])
    C(센터 대기자) --> UC2([매일 숏폼 미션 수행])
    B(데이터형 가족) --> UC3([시계열 리포트 열람])
    B --> UC4([성장 뱃지 가족 공유])
    
    T(보육 교사) --> UC5([Zero-touch 오디오 수집])
    T --> UC6([AI 쿠션어 알림장 승인])
    
    D(유치원 원장) --> UC7([엑셀 일괄 등록 및 동의서])
    D --> UC8([원장 대시보드 관리])
...
클라이언트부터 코어 파이프라인, 외부 API로 이어지는 컴포넌트 아키텍처입니다.
...
flowchart TD
    subgraph Client Environments
        M[B2C Mobile App\nReact Native]
        W[B2C Web App\nReact]
        T[B2B Teacher Tablet\nEdge VAD]
        A[B2B Director Admin\nVue]
    end
    ...
    subgraph Core Backend Services
        ...
        Mission[Curriculum & Mission DB]
        B2B_Svc[B2B Management DB]
    end

    subgraph AI Pipeline
        STT[Speech-to-Text]
        Diarize[Speaker Diarization]
        Score[3-Axis Scoring Engine]
        LLM[LLM Cushion Word Generator]
    end

    subgraph HITL System
        HQ[(Queue DB)]
        Admin[Expert Panel Web]
    end
    ...
    M & W & T & A --> GW
    GW --> Auth & Mission & B2B_Svc
    
    T -- Audio Stream --> Diarize
    M & W -- Audio Stream --> STT
    
    Diarize --> STT
    STT --> Score
    Score --> Mission
    Score -. Low Confidence .-> HQ
    HQ --> Admin
...
### 3.5 Interaction Sequences (핵심 플로우)
...
| **REQ-FUNC-F5-002** | API 장애 Fallback | 외부 API 통신 실패 혹은 타겟 단말기 환경 제약 시 대체 방식을 제공 파 제공한다. | **Exc:** 카카오톡 공유 실패 시 디바이스 클립보드에 "링크 복사" 기능으로 자동 폴백(Fallback). | S3-Neg2 | P1 / Should |
...
| **REQ-NF-PERF-02** | 스트리밍 전송 지연 | 모바일 엣지에서 서버로의 오디오 청크 전송 지연시간(Latency)은 `≤ 300ms`를 유지해야 한다. | S1-AC2 |
...
erDiagram
    ...
    USER ||--o{ SESSION_LOG : generates
    USER ||--o| REWARD_PROGRESS : owns
    USER ||--o{ WEEKLY_REPORT : receives
    SESSION_LOG ||--|| EVALUATION_RESULT : produces
    MISSION_CARD ||--o{ SESSION_LOG : references
    ...
    EVALUATION_RESULT {
        ...
        int articulation_score
        int linguistic_score
...
#### 6.2.3 Class Diagram (Core Domain)
```mermaid
classDiagram
    class User {
        +String userId
    ...
    SessionLog "1" *-- "1" EvaluationResult
    MissionCard "1" *-- "many" SessionLog
```
...
#### 6.3.2 게이미피케이션 및 네트워크 소급 플로우 (Reward Fallback)
```mermaid
sequenceDiagram
    autonumber
    actor Child
...
#### 6.3.3 전문가 HITL 에스컬레이션 플로우
```mermaid
sequenceDiagram
    autonumber
    participant AI as 3축 AI Engine
    ...
    alt Confidence < 70%
        AI->>Q: 자동 에스컬레이션 티켓 생성
    end
    
    Q->>Expert: 긴급 리뷰 할당 알림
    
    par 48시간 타이머 작동
        SL->>Q: 티켓 체류 시간 트래킹
        alt 48h 초과
            SL->>Q: 마스터 재활사로 강제 이관 (SLA 초과)
        end
    and 전문가 리뷰
        Expert->>Q: 오디오 스트리밍 청취 및 코멘트 작성
        Expert->>Q: 보정 점수 및 Ground Truth 입력
    end
    
    Q->>User: 전문가 감수 결과 도착 푸시 알림
    Q->>AI: 보정 데이터 500건 누적 시 MLOps 재학습 트리거
```
