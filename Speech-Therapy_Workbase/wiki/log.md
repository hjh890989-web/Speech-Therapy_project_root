...
## [2026-06-08] note | F1-a/F4 임상설계 reference 신설 — NISE-B·ACT 구인 기반 앱 원본 기능 설계
- pillar: product
- 추가: wiki/product/concepts/F1a-F4-임상설계-reference.md — NISE-B·ACT **전수 구조**(읽기 5·쓰기 3·수학 4 소검사) + **채점 척도**(읽기 0/1·쓰기 0/1/2·수학 0/1, RAN·유창성 시간/개수)를 정리하고, 이를 근거로 앱 **F1-a/F4/F15 원본 기능 설계 가이드** 작성(구인→자체 콘텐츠 제작 지침→자체 채점→기능 매핑).
- ⚠️ 저작권 경계 명시 = 핵심: 구인·방법론(분야 표준 지식)은 설계 reference로 활용 / 문항·지문·규준·"NISE-B·ACT" 명칭은 **미사용(자체 원본 제작)**. 상업 출시 전 명칭·규준 미사용 + 원본성 법률 검토 권고.
