...
| **V10 후보** 🆕 | (미작성) | CR-2026-007 | **읽기 발달 선행지표 가치 확장** — F1-a 를 "조음·TTR·MLU" → **"만 5-7세 읽기 준비도(음운인식·해독·RAN) 측정"** 으로 확장(새 JTBD "학교 가기 전 읽기 준비됐나?") + F4.1 음소 핀셋에 음운인식 축 + F15.1 추론 4수준 위계. 델타 정본 = `Speech-Therapy_App/tasks/11_Literacy_Constructs_Expansion_Design.md` §6-B. ⚠️ **V10 정본으로 대체됨** — CR-007 "읽기 준비도 측정" 프레임은 CR-009 라이브(연습-only)와 충돌 → 아래 V10 행이 "측정"을 "놀이·연습"으로 정정 |
| **V10 정본** ✅ | (신규) `Speech-Therapy_App/docs/VPS_V10_TwoTrack_Realign.md` | 2트랙 재정렬 (CR-2026-009 라이브 반영) | **= 2트랙 비대칭 재정렬(CR-009 라이브 반영)** — 발음 단일트랙·측정 정점(V09)을 트랙A(발음 "확인", 만2~7, U-TAP·SELSI 규준) + 트랙B(문해 "놀이·연습", 만2~12, 14게임 S0~S4, 산출=engagement·`referenceBand=null`·전단계 `bandShippable=false`) **비대칭 정본**으로 신설. 트랙B 는 점수·밴드·또래백분위·판정 **없음**("확인(probe)"은 발음 전용). 북극성 = 트랙A W-AUR(≥60% 불변) + 트랙B W-LER(주간 문해 활동률, engagement, 보조지표 신설 → 옵션C 동격 승격). 문해 4게이트(연습-only/비복제/플래그게이팅/연령도메인분리). 근거 = `docs/realignment/00_2track_realignment_blueprint.md` §3·§6·§7·§8. CON-04 신규 카피 위반 0 |
